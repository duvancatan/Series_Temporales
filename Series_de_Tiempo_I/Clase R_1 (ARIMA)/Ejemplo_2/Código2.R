...
ts.plot(z)
lz<-log(z)
plot.ts(lz, main="Serie", ylab="");grid()
par(mfrow=c(1,2))
acf(lz, lag.max=30, main="ACF")
pacf(lz,lag.max=30, main="PACF")
...
########################
rt<-diff(lz) #Retornos
plot.ts(rt, ylab="", main="Serie 5 Diferenciada");grid()
par(mfrow=c(1,2))
acf(rt, lag.max=25, main="")
pacf(rt, lag.max=25, main="")
...
?arima
(mod_1=arima(lz, c(0, 1, 1), method = c("CSS-ML"))) 
...
mod<-arima(lz, order = c(3, 1, 0),method = c("CSS-ML"))
forecast(mod)
...
lz.pred<-predict(mod,n.ahead=30)
lines(lz.pred$pred,col="blue", lty=3, lwd=2)
lines(lz.pred$pred+2*lz.pred$se,col="red",lty=3, lwd=2)
lines(lz.pred$pred-2*lz.pred$se,col="red",lty=3, lwd=2)
