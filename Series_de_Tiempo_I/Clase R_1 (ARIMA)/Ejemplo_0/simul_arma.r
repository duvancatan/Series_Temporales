...
(modar10)
names(modar10)
med1<-modar10$x.mean # Media estimada
var1<-modar10$asy.var.coef # Var Asint Estimada de Parametros
sd1<-sqrt(var1)

#2#
?arima
modar11<-arima(z_ar1, order = c(1,0,0))
summary(modar11)
names(modar11)

#3#
?sarima
modar12<-sarima(z_ar1, p=1,d=0,q=0)
summary(modar12)
names(modar12)
...
# Simulacion
z_ar2=arima.sim(n = 144, list(ar = c(phi1_ar2, phi2_ar2)), mean=theta0_ar2, sd = sqrt(var_RB_ar2), n.start=20)
...
?ar
modelo<-ar(z_ar2, aic = TRUE)
modelo
sarima(z_ar2, p=2, d=0, q=0)
############################################################################
#---------------------------------------------------------------------------
